...
-(void)shareViewShareBtnClickWithTag:(NSInteger)tag{
    if (tag == 10) {
       ...
       BOOL isInstall = [[UMSocialManager defaultManager]isInstall:UMSocialPlatformType_WechatSession];
        ...
        //朋友圈
        BOOL isInstall = [[UMSocialManager defaultManager]isInstall:UMSocialPlatformType_WechatTimeLine];
        if (isInstall) {
        ...
        //QQ空间
        BOOL isInstall = [[UMSocialManager defaultManager]isInstall:UMSocialPlatformType_Qzone];
        if (isInstall) {
...
//    NSString* thumbURL =  @"https://mobile.umeng.com/images/pic/home/social/img-1.png";
    UMShareWebpageObject *shareObject = [UMShareWebpageObject shareObjectWithTitle:self.shareTitle descr:self.shareDesion thumImage:SetImage(@"佣兵天下APP图标 Copy 3")];
    //设置网页地址
...
-(void)setShareDesion:(NSString *)shareDesion{
    _shareDesion = shareDesion;
}

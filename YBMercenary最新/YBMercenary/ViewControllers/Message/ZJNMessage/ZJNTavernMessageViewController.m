//
//  ZJNTavernMessageViewController.m
//  YBMercenary
//
...
@interface ZJNTavernMessageViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation ZJNTavernMessageViewController
...
-(void)getDataFromService{
    ...
    [[YBRequestManager sharedYBManager] postWithUrlString:UserCenter_post_get parameters:dic success:^(id data) {
        if ([[data[@"code"] stringValue] isEqualToString:@"0"]) {
            ...
            for (int i = 0; i <arr.count; i ++) {
                ZJNJiuGuanModel *model = [ZJNJiuGuanModel yy_modelWithDictionary:arr[i]];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.secondModel = self.dataArr[indexPath.row];
    return cell;
}
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
//    ZJNJiuGuanModel *model = self.dataArr[indexPath.row];

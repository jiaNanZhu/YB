...
@interface PublicCustomTaskVC ()<PublishTaskRequireCellDelegate,ZJNPublishTaskMaskCellDelegate>
...
{
    NSString *payAmountStr = self.task_nameCell.moneyField.text;
    CGFloat payAmount = 100*[payAmountStr floatValue];
    
    NSDictionary *parameters =@{@"token":USERTOKEN,@"task_type":@(self.task_type),@"task_type_child":@(self.task_type_child),@"task_name":self.task_nameCell.nameField.text,@"pay_amount":[NSString stringWithFormat:@"%.f",payAmount],@"task_request":self.task_request,@"task_tag":self.task_tag,@"task_description":self.detailCell.textView.text,@"task_img":self.imagePath,@"task_city":[[NSUserDefaults standardUserDefaults] objectForKey:@"locationCity"]};

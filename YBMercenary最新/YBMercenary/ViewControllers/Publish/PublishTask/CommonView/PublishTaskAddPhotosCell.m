...
@interface PublishTaskAddPhotosCell()<UICollectionViewDelegate,UICollectionViewDataSource,PhotosNavigationControllerDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate>
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.deleteimg = ^{
        ...
        NSMutableArray *imagePathArr = [NSMutableArray arrayWithArray:[self.imagePaths componentsSeparatedByString:@"|"]];
        [imagePathArr removeObjectAtIndex:indexPath.row];
        self.imagePaths = [imagePathArr componentsJoinedByString:@"|"];
        if (self.imageUrlsBlock) {
            self.imageUrlsBlock(self.imagePaths);
        }
    ...
    [[YBRequestManager sharedYBManager] postWithUrlString:ImageUpload parameters:@{@"token":USERTOKEN} images:self.dataArr success:^(id data) {
        ...
        if ([[data[@"code"] stringValue] isEqualToString:@"0"]) {
            ...
            for (NSDictionary *dic in array) {
                NSString *path = dic[@"post"];
                [tempArr addObject:path];
            }
            self.imagePaths = [tempArr componentsJoinedByString:@"|"];
            if (self.imageUrlsBlock) {
                self.imageUrlsBlock(self.imagePaths);
            }
        }else{

...
-(void)requestCommonDrunkery_post_add
{
    ...
    NSDictionary *parameters =@{@"token":USERTOKEN,@"task_type":@(self.task_type),@"task_type_child":@(self.task_type_child),@"task_name":self.task_nameCell.nameField.text,@"task_purpose":self.task_purposeCell.textView.text,@"task_request":self.task_request,@"pay_amount":[NSString stringWithFormat:@"%.f",payAmount],@"task_tag":self.task_tag,@"task_description":self.detailCell.textView.text,@"task_img":@"",@"task_city":[[NSUserDefaults standardUserDefaults] objectForKey:@"locationCity"]};

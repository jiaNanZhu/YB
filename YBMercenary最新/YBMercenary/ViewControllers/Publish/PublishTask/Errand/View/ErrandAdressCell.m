...
- (void)creatSubViews{
    ...
    UILabel *arrive = [F_UI SL_UI_Label:@"开始地址" color:[UIColor hex:@"444444"] textAlignment:0 textFont:AdFloat(28) numberOfLines:1];
    ...
    UILabel *valid = [F_UI SL_UI_Label:@"目的地址" color:[UIColor hex:@"444444"] textAlignment:0 textFont:AdFloat(28) numberOfLines:1];
    ...
    _goalLabel = [F_UI SL_UI_Label:@"请选择目的地地址" color:[UIColor hex:@"999999"] textAlignment:NSTextAlignmentLeft textFont:AdFloat(28) numberOfLines:1];
    ...
    _beginLabel = [F_UI SL_UI_Label:@"请选择取货地址" color:[UIColor hex:@"999999"] textAlignment:NSTextAlignmentLeft textFont:AdFloat(28) numberOfLines:1];
...
//    NSArray *defaultSelArr=nil;
    if (sender.view.tag == 11111) {
         ...
         */
        if (self.delegate && [self.delegate respondsToSelector:@selector(errandAddressCellSelectStardAddress)]) {
            [self.delegate errandAddressCellSelectStardAddress];
        }
    }
    else
    {
       ...
       defaultSelArr = [ self.goalLabel.text componentsSeparatedByString:@" "];
         */
        if (self.delegate && [self.delegate respondsToSelector:@selector(errandAddressCellSelectEndAddress)]) {
            [self.delegate errandAddressCellSelectEndAddress];
        }
    }
    /*
...
- (void)deleteAction{
    if (self.delegate && [self.delegate respondsToSelector:@selector(errandAddressCellDeleteStartAddress)]) {
        [self.delegate errandAddressCellDeleteStartAddress];
    }
}

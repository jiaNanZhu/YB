...
-(void)makeUI{
    ...
    headIMG = [F_UI SL_UI_UIimg:[UIImage imageNamed:@"Group1"] cornerRadius:AdFloat(60)];
...
//        UILabel *countLabel = [[UILabel alloc]initWithFrame:CGRectMake(AdFloat(70), 5, AdFloat(16), AdFloat(16))];
...
//    }
        ...
        DBLog(@"------ 我的推广 ------");
        if ([self.user.isagree isEqualToString:@"0"]) {
            ZJNAgreeRulesViewController *viewC = [[ZJNAgreeRulesViewController alloc]init];
            viewC.hidesBottomBarWhenPushed = YES;
            [self.navigationController pushViewController:viewC animated:YES];
        }else{
            YBMyGeneralizeViewController *viewC = [[YBMyGeneralizeViewController alloc]init];
            viewC.title = @"我的推广";
            viewC.urlStr = [NSString stringWithFormat:@"http://yb.dashuibei.com/register/extension.html?token=%@",USERTOKEN];
            viewC.hidesBottomBarWhenPushed = YES;
            [self.navigationController pushViewController:viewC animated:YES];
        }
    ...
    if ([[YBUser boolLogin] isEqualToString:@""]||[YBUser boolLogin] == nil) {
        ...
        [[YBRequestManager sharedYBManager]postWithUrlString:UserCenter_getinfo parameters:parameters success:^(id data) {
            ...
            if ([data[@"code"]integerValue] ==0) {
                ...
                RCUserInfo *_currentUserInfo =[[RCUserInfo alloc] initWithUserId:self.user.id name:self.user.name portrait:[NSString stringWithFormat:@"%@%@",HOST,self.user.head_img]];
                [RCIM sharedRCIM].currentUserInfo = _currentUserInfo;
                
                [headIMG sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",HOST,user.head_img]] placeholderImage:SetImage(@"Group1")];

...
@implementation ZJNMyIssueWTGCell

+(instancetype)creatTableViewCellWithTableView:(UITableView *)tableView{
    static NSString *cellid = @"ZJNMyIssueWTGCell";
    ZJNMyIssueWTGCell *cell = [tableView dequeueReusableCellWithIdentifier:cellid];
    if (!cell) {
        cell = [[ZJNMyIssueWTGCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellid];
...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        ...
        [self.infoLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.edges.equalTo(bgview).with.insets(UIEdgeInsetsMake(AdFloat(20), AdFloat(30), AdFloat(20), AdFloat(30)));
        ...
        make.top.equalTo(self.lineView.mas_bottom).offset(AdFloat(18));
            make.right.equalTo(self.contentView).offset(-AdFloat(30));
            make.size.mas_equalTo(CGSizeMake(AdFloat(120), AdFloat(52)));
...
-(void)setModel:(YBMyIssueModel *)model{
    ...
    if ([self.model.task_status isEqualToString:@"5"]) {
        //后台审核未通过
        self.infoLabel.text = [NSString stringWithFormat:@"任务被驳回原因：%@",self.model.refuse_cause];
        [Utile setUILabel:self.infoLabel data:nil setData:@"任务被驳回原因：" color:[UIColor hex:@"666666"] font:AdFloat(28) underLine:NO];
        self.stateLabel.text = @"后台审核未通过";
        [self.stateLabel mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(self.lineView.mas_bottom).offset(AdFloat(18));
            make.right.equalTo(self.contentView).offset(-AdFloat(30));
            make.size.mas_equalTo(CGSizeMake(AdFloat(240), AdFloat(52)));
        }];
    }else{
        self.infoLabel.text = [NSString stringWithFormat:@"拒绝原因：%@",self.model.refuse_cause];
        [Utile setUILabel:self.infoLabel data:nil setData:@"拒绝原因：" color:[UIColor hex:@"666666"] font:AdFloat(28) underLine:NO];
    }

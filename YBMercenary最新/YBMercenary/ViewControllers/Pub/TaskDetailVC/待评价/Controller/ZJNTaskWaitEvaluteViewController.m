//
//  ZJNTaskWaitEvaluteViewController.m
//  YBMercenary
//
...
#import "UserInfoVC.h"
@interface ZJNTaskWaitEvaluteViewController ()<UITableViewDelegate,UITableViewDataSource,SDPhotoBrowserDelegate>
@property (nonatomic ,strong)UITableView *tableView;
@property (nonatomic, strong)TaskDetailModel *detailModel;
@property (nonatomic, strong)NSArray *messageArr;//保存留言信息数组
@property (nonatomic, strong)ZJNEvaluateBtnView *bottomBtn;

@end

@implementation ZJNTaskWaitEvaluteViewController
...
-(void)getDataFromService{
    ...
    [[YBRequestManager sharedYBManager] postWithUrlString:Get_taskdetail parameters:dic success:^(id data) {
        ...
        if ([data[@"code"] integerValue] == 0) {
            ...
            if ([self.detailModel.task_status isEqualToString:@"6"]) {
                [self.bottomBtn.btn setTitle:@"去评价" forState:UIControlStateNormal];
            }else if ([self.detailModel.task_status isEqualToString:@"7"]){
                [self.bottomBtn.btn setTitle:@"已评价" forState:UIControlStateNormal];
                self.bottomBtn.btn.userInteractionEnabled = NO;
            }
...
#pragma mark-创建底部视图
-(ZJNEvaluateBtnView *)bottomBtn{
    if (!_bottomBtn) {
        _bottomBtn = [[ZJNEvaluateBtnView alloc]init];
        
        [_bottomBtn.btn addTarget:self action:@selector(bottomBtnClick) forControlEvents:UIControlEventTouchUpInside];
    }
    return _bottomBtn;
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        ...
        if ([dic[@"id"] isEqualToString:userId]) {
        ...
        [cell.headImageV sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",HOST,dic[@"head_img"]]] placeholderImage:SetImage(@"Group1")];
...
-(void)bottomBtnClick{
    ...
    view.evaluteStarBlock = ^(NSInteger stars) {
        [self getDataFromServiceWithStars:stars];
    };
    [[UIApplication sharedApplication].keyWindow addSubview:view];
}
-(void)getDataFromServiceWithStars:(NSInteger)stars{
    NSDictionary *dic = @{@"token":USERTOKEN,@"id":@(self.taskId),@"star":@(stars)};
    [[YBRequestManager sharedYBManager] postWithUrlString:Pingjia parameters:dic success:^(id data) {
...
-(void)chatButtonClick{
    NSDictionary *dic = self.detailModel.xuanding;
    [self pushToChatViewControllerWithChatId:dic[@"id"] title:dic[@"name"]];
}

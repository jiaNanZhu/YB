...
#import "UserInfoVC.h"
@interface ZJNWhileTaskViewController ()<UITableViewDelegate,UITableViewDataSource,ZJNSignUpBottomViewDelegate,SDPhotoBrowserDelegate>
...
@implementation ZJNWhileTaskViewController
...
-(void)getDataFromService{
    ...
    [[YBRequestManager sharedYBManager] postWithUrlString:Get_taskdetail parameters:dic success:^(id data) {
        ...
        if ([data[@"code"] integerValue] == 0) {
            ...
            if (self.taskType == 0) {
                [self.bottomView.stateBtn setTitle:@"已被领取" forState:UIControlStateNormal];
                self.bottomView.stateBtn.userInteractionEnabled = NO;
            }else{
                [self.bottomView.stateBtn setTitle:@"提交任务" forState:UIControlStateNormal];
            }
...
-(void)bottomStateBtnClick{
     ...
     [[YBRequestManager sharedYBManager] postWithUrlString:Tijiao parameters:dic success:^(id data) {
         if ([[data[@"code"] stringValue] isEqualToString:@"0"]) {
             ...
             [self.bottomView.stateBtn setTitle:@"已提交审核" forState:UIControlStateNormal];
             if (self.whileTaskRefreshBlock) {
                 self.whileTaskRefreshBlock();
             }
         }else{
             [DWBToast showCenterWithText:data[@"msg"]];
         }

...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        if (indexPath.row == 0) {
            ...
            if (self.type == 1) {
                cell.priceLabel.text = [NSString stringWithFormat:@"有效期：%@天",self.detailModel.validity_time];
            }else{
                cell.priceLabel.text = @"";
            }
            NSString *taskTag = [NSString stringWithFormat:@"%@",self.detailModel.task_tag];
            NSLog(@"%@",taskTag);
            if(self.detailModel.task_tag !=nil && self.detailModel.task_tag.length !=0 && ![self.detailModel.task_tag isEqual:@""]&& self.detailModel.task_tag != NULL && ![self.detailModel.task_tag isEqualToString:@""]){
        ...
        if (self.type == 1) {
            ...
            NSString *timeStr = [DateTool timeStampToString:[self.detailModel.delivery_time integerValue]];
            cell.timeLabel.text = [NSString stringWithFormat:@"%@天",timeStr];
            ...
            cell.validityLabel.text = [NSString stringWithFormat:@"%@天",self.detailModel.validity_time];
            cell.purposeLabel.text = self.detailModel.task_purpose;
            cell.requireLabel.text = self.detailModel.task_request;
            cell.detailLabel.text = self.detailModel.task_description;
            return cell;
        ...
        [cell.iconImage sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",HOST,dic[@"apply_user_headimg"]]] placeholderImage:[UIImage imageNamed:@"Group1"]];
        cell.nameLabel.text = dic[@"apply_user_name"];
        cell.priceLabel.hidden = YES;
        NSString *userId = [[NSUserDefaults standardUserDefaults]objectForKey:@"id"];
        if ([userId isEqualToString:dic[@"apply_user_id"]]) {
            cell.btnTitle = @"放弃";
        }else{
            cell.btnTitle = @"剔除";
        }
...
-(void)shareBtnClick{
    ...
    self.shareUrl = [NSString stringWithFormat:@"http://yb.dashuibei.com/register/detail.html?id=%@",self.detailModel.id];
    self.shareTitle = self.detailModel.task_name;
    self.shareDesion = self.detailModel.task_purpose;
    [self showShareView];
}

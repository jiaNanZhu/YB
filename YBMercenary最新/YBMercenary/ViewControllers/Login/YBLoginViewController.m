...
//    [loginBtn setBackgroundColor:[UIColor hex:@"0386FF"]];
    [loginBtn setBackgroundImage:SetImage(@"nav") forState:UIControlStateNormal];
...
//获取用户信息
-(void)requestuserInfoWithData:(id)data{
    [KVNProgress show];
    NSDictionary *parameters =@{@"token":data[@"data"][@"token"]};
...
-(void)loginRCloudwith:(id)data{
    [[RCIM sharedRCIM] connectWithToken:data[@"data"][@"yun_token"] success:^(NSString *userId) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [self requestuserInfoWithData:data];

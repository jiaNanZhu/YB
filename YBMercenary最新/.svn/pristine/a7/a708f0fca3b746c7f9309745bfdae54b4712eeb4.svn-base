...
@interface PublishTaskDetailCell ()<UITextViewDelegate>
...
@implementation PublishTaskDetailCell

+ (instancetype)creatTableViewCellWithTableView:(UITableView *)tableView{
    PublishTaskDetailCell *cell = [tableView dequeueReusableCellWithIdentifier:@"PublishTaskDetailCell"];
    if (cell == nil) {
        cell = [[PublishTaskDetailCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"PublishTaskDetailCell"];
...
- (void)creatSubViews{
    ...
    UILabel *taskGoal = [F_UI SL_UI_Label:@"任务详情" color:[UIColor hex:@"444444"] textAlignment:0 textFont:14 numberOfLines:1];
    ...
    _countLabel = [F_UI SL_UI_Label:@"0/200" color:[UIColor hex:@"999999"] textAlignment:0 textFont:14 numberOfLines:1];
    ...
    _tipLabel = [F_UI SL_UI_Label:@"请输入任务详细介绍" color:[UIColor hex:@"999999"] textAlignment:0 textFont:14 numberOfLines:1];
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text{
    ...
    if (textView.text.length >= 200) {
        [DWBToast showCenterWithText:@"介绍文字不得超过200个字符"];
        return NO;
    }
    return YES;
}
...
- (void)textViewDidChange:(UITextView *)textView{
    if (textView.text.length > 0) {
        ...
        if (self.detailBlock) {
            self.detailBlock(textView.text);
        }

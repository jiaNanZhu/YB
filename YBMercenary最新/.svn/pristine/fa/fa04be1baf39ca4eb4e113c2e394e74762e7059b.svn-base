...
- (void)configCollectionView {
    ...
    if (self.choseSingleImage) {
        _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(margin, top, CGRectGetWidth(self.view.frame) - 2 * margin, CGRectGetHeight(self.view.frame) - top) collectionViewLayout:layout];
    }else{
...
//    [KVNProgress showWithStatus:@"" onView:[UIApplication sharedApplication].delegate.window];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.model = model;
    cell.choseSingleImage = self.choseSingleImage;
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        ...
        if (isSelected) {
            ...
            if (weakSelf.pickerModelArray.count < navigation.maxImagesCount) {
                ...
                [navigation showAlertWithTitle:[NSString stringWithFormat:@"你最多只能选择9张照片"]];
            }
        }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.type == AlbumModelMediaTypeVideo) {
        ...
        photoPreviewVc.choseSingleImage = self.choseSingleImage;
        [self pushPhotoPrevireViewController:photoPreviewVc];
    }
}
